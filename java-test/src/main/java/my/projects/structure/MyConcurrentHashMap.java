 ...
 * same mapping value.
 *
 * <p>A MyConcurrentHashMap can be used as scalable frequency map (a
 ...
 * and/or return values. Because the elements of a MyConcurrentHashMap
...
public class MyConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)    // if table is null, initiate table
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {
                // if node is null, cas to create the node.
                // if cas succeed, break and add count.
                // if cas failed, continue to next loop; in next loop, the node is already created,
                    ...
                    if (tabAt(tab, i) == f) {   // check if node changed
                        if (fh >= 0) {   // search from linked node
                            ...
                            for (Node<K,V> e = f;; ++binCount) {
                                ...
                                Node<K,V> pred = e;
                                if ((e = e.next) == null) {   // if key not existed, create new node
    ...
    private static void testCapacity1() {
        // default size 16, load factor 12
    ...
    private static void testCapacity2() {
        // initial capacity 100, power-of-2 bound is 128; however 100 > 128*localFactor,
        // thus the initial table size is 256
        MyConcurrentHashMap<String, String> map = new MyConcurrentHashMap<>(100);
        map.put("a", "");
        map.put("b", "");
        map.put("c", "");
        System.out.println(map.sizeCtl);
